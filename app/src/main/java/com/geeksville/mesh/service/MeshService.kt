...
class MeshService : Service(), Logging {
    ...
    private fun handleReceivedData(packet: MeshPacket) {
        myNodeInfo?.let { myInfo ->
            ...
            if (dataPacket != null) {
                ...
                //
                var tempStr = dataPacket.text
                debug("RECEIVED MESSAGE=${tempStr}")
                ...
                //             we will stored the data packets.
                //          02-"REFRESH-LOCATION" message are ONLY for GPS own and GPS remote instants
                //              update. No need to add to the data packet
                /*
                if (tempStr != "REFRESH-LOCATION")
                ...
                */
                ...
                // Modified By: Ahmad Bahari
                // Date: 10/12/2021
                // Remarks: Comments out original code segments
                rememberDataPacket(dataPacket)
                ...
                when (data.portnumValue) {
                    Portnums.PortNum.TEXT_MESSAGE_APP_VALUE ->
                        if (!fromUs) {
                            ...
                            // Date: 10/12/2021
                            // Remarks: 01-Filter incoming messages, where others than "REFRESH-LOCATION" message
                            //             we will update the android notifications.
                            //          02-"REFRESH-LOCATION" message are ONLY for GPS own and GPS remote instants
                            //              update. No need to add to the data packet
                            /*
                            if (tempStr != "REFRESH-LOCATION")
    ...
    private fun onConnectionChanged(c: ConnectionState) {
        ...
        fun startConnect() {
            ...
            try {
                ...
                // Modified By: Ahmad Bahari
                // Date: 08/12/2021
                // Remarks: Test timer process for startConfig request flow.
                // Date: 11/12/2021
                // Remarks: Comments out the code segments. Attempt to get the OWN GPS
                //          but failed!.
                /*
                if (!startTimer){
                    repeater()
    ...
    // Modified By: Ahmad Bahari
    // Date: 08/12/2021
    // Remarks: 01-Indirectly reset controller by issuing startConfig command.
    //          02-Initiate from command button located at settings_fragment tab.
    // Date: 11/12/2021
    // Remarks: NOT used because an attempt to get our OWN GPS was failed!
    //          but failed!.
    fun resetDevice(){
        resetStart = true
    }
    ...
    //          but failed!.
    private fun cDTimer(){
        if (!isActionAchieved && secondsPassed < 2){  // repeat check if Action NOT Achieved for max of 10 seconds
            Handler(Looper.getMainLooper()).postDelayed({
                //debug("Timer START")
                repeater()
                secondsPassed++
                if (secondsPassed == 2 && resetStart){
                    debug("Send COMMAND to RADIO")
                    /*
                    if (configNonce != 0){
                        configNonce = 1
                    }
                ...
                else if (secondsPassed == 2 && !resetStart){
                    secondsPassed = 0
                }
    ...
    // Modified By: Ahmad Bahari
    // Date: 08/12/2021
    // Remarks: Add the function call to the time out function
    // Date: 11/12/2021
    // Remarks: NOT used because an attempt to get our OWN GPS was failed!
    //          but failed!.
